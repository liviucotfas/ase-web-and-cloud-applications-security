...
5. Add the following `Category` class to the `Models` folder:
    ...
    > The `Category` class represents a product category. The `Products` navigation property establishes a one-to-many relationship: one category can have many products.

6. Add the following `Product` class to the `Models` folder:
    ...
    > The `CategoryID` property is a foreign key that establishes the relationship between `Product` and `Category`. Each product belongs to one category.
...
7. Add a `Data` folder to the project. Add a class file called `ApplicationDbContext.cs` to the `Data` folder and defined the class shown below.
    ...
    The `DbContext` base class provides access to the Entity Framework Core's underlying functionality. The `Products` property will provide access to the `Product` table in the database, and the `Categories` property will provide access to the `Category` table.
...
8. Modify the `Main` method in the `Program` class as follows.
...
16. To populate the database and provide some sample data, let's add a class file called `SeedData.cs` to the `Data` folder.
    ...
    > Further details: https://docs.microsoft.com/en-us/aspnet/core/tutorials/razor-pages/sql
    ...
    > The static `EnsurePopulated` method receives a `WebApplication` argument, which is the interface used in the `Main` method of the `Program` class to register middleware components to handle HTTP requests. 
    ...
    > The `EnsurePopulated` method obtains an `ApplicationDbContext` object through the `WebApplication` interface and calls the `Database.Migrate` method if there are any pending migrations, which means that the database will be created and prepared so that it can store `Category` and `Product` objects.

    > **Important:** The seeding process follows a two-step approach:
    > 1. **Seed categories first**: Categories are created and saved to the database. This ensures they have valid `CategoryID` values assigned by the database.
    > 2. **Seed products with category references**: After categories exist, products are created with valid `CategoryID` foreign keys that reference the existing categories.
    
    > The method checks if there are any objects in the database using the `Any()` method. If the database is empty, it populates it with sample data relevant to a security course context. The security-themed products (antivirus, password managers, training courses) make the application more meaningful when demonstrating authentication, authorization, and other security concepts.
